...
def read_cross_section(lambdalist):
    ...
    x, y = np.loadtxt(fname=os.path.dirname(os.path.abspath(__file__)) + "/data/Aluminium/AlCrossSect_(n,n).py",
    ...
    # removing all values that appear more than once because interp1d crashes otherwise (needs strict monotony)
    diff_xlog = np.diff(np.array(xlog))
    ## collecting all indexes with non-zero difference (to be dropped)
    nums_drop = []
    for j, diff in enumerate(diff_xlog):
        if diff == 0:
            nums_drop.append(j)
    xlog = np.delete(xlog, nums_drop)
    ylog = np.delete(ylog, nums_drop)
