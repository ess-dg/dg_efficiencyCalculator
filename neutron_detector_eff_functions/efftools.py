...
def metadata_diffthick_vs_wave(sigmaeq, blades, ranges, varargin, nb):
	eff = []
	...
	for sigma in sigmaeq:
			effd = mgeff_depth_profile(thickness, ranges, sigma[0], varargin)[1]
			eff.append(effd*100)
	return eff
...
def mgeff_depth_profile(thickness, ranges, sigma, varargin):
	...
	eff =[]
	efftotal = 0
	for i, t in enumerate(thickness): # loop over blades
		expi = exp(-2*sigma*cumthick)
		eff.append([eff1blade[i][0]*expi, eff1blade[i][1]*(delta**i)*expi])
		#efftotal = efftotal + eff[i][0]
		# use [1] because it contains BS + T + substrate
		efftotal = efftotal + eff[i][1]
		cumthick = cumthick+t
	# eff: efficiency for each blade for [0] and [1]
	# efftotal: efficiency for all blades for [1]
	return eff, efftotal
...
..  Original source in Matlab: https://bitbucket.org/europeanspallationsource/dg_matlabborontools/src/bcbac538ad10d074c5150a228847efc2e0269e0d/MultiGrid_Optimization/MG1_Calc4monoch_sameThickBlades_VS_ThickAndNb.m?at=default&fileviewer=file-view-default
	...
	if varargin is None:
		...
		delta = varargin
	#print("efficparam varargin", varargin)
	c1 = efficiency2particles(thickness,ranges[0],ranges[1],sigma_eq) # 94% alpha, Li (+ gamma)
	d1 = efficiency2particles(thickness,ranges[2],ranges[3],sigma_eq) # 6% alpha, Li
	# [0] backscattering, [1] transmission
	efficiency = []
	# backscatt
