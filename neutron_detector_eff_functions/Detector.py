...
class Detector:
    ...
    def calculate_eff(self):
        ...
        print("aluminium")
        print("varargin in Detector", varargin)
        result = []
        if self.single:
    ...
    def plot_wave_vs_eff(self,sigmaeq, sigmalist, ranges, varargin, blades, result, wavelength, figure):
        ...
        if self.single:
            ...
            y = efftools.metadata_diffthick_vs_wave(sigmaeq, blades, ranges, varargin, len(blades))
        cx = figure.add_subplot(111)
        self.metadata.update({'effVsWave': [sigmalist, y]})
        print("plot_wave_vs_eff", varargin)
        ...
        cx.grid(True)
        cx.set_xlabel('Neutron wavelength ($\AA$)')
    ...
    def calculate_varargin(self, blades_subs, wavelength, angle):
        #thick = self.blades[0].substrate
        varargin = Aluminium.aluminium(blades_subs, wavelength, angle)
        print(varargin)
